...
class DefaultCodingAgent implements CodingAgent, HasProviderOptions
{
    ...
    public function instructions(): string
    {
        ...
        - After CreatePullRequest or CommitAndPush returns, report its result exactly. Never replace the returned error with a guessed cause such as missing credentials. If it failed, quote the failure and give next steps only when the result supports them. CreatePullRequest pushes before calling GitHub's API, so an HTTP PR-creation error never means the branch push failed.
