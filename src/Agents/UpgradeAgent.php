...
class UpgradeAgent implements CodingAgent, HasProviderOptions
{
    ...
    public function instructions(): string
    {
        ...
        6. **Deliver.** Summarise honestly: what changed, which breaking changes you addressed, which upgrade-guide items did NOT apply to this app, and — importantly — what the test suite does NOT cover, so a green run is not oversold. Then call ConfirmAction and, if approved, CreatePullRequest with a branch like tackle/upgrade-<package>-v<major>, that honest summary as the body, and `files` listing every individual repository-relative file changed by the upgrade. Never pass a directory. If the session context says other packages are queued for their own upgrade sessions, add a merge-order note to the PR body: each upgrade PR touches composer.lock, so whichever merges second must be rebased and have composer update re-run on its branch.
